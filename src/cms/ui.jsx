...
export const inputClass =
  'w-full rounded-lg border border-white/10 bg-[#101119] px-3 py-2 text-[.85rem] text-white/90 outline-none transition-colors placeholder:text-white/25 focus:border-[#7c5cff] focus:bg-[#12131d]';
...
export function Panel({ title, subtitle, right, children, className = '' }) {
  return (
    <section className={`rounded-xl border border-white/8 bg-[#0e0f16] ${className}`}>
      {(title || right) && (
        <header className="flex items-center justify-between gap-3 border-b border-white/8 px-3.5 py-2.5">
