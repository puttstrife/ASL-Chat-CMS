...
// Opt-in "Talk to Marisol live" — ElevenLabs Conversational AI.
      ...
      try {
        ...
        convRef.current = await Conversation.startSession({
          ...
          onModeChange: ({ mode }) => {
            ...
            setStatus(mode === 'speaking' ? 'Marisol is speaking…' : 'Listening…');
          },
          onError: () => { setStatus('Connection lost. Returning to the thread.'); setTimeout(onClose, 1800); },
        });
      } catch {
        if (active) { setStatus('Live voice is unavailable right now. Marisol will stay with you here.'); setTimeout(onClose, 2200); }
      }
    })();
