...
// `persona` supplies the face and the tab title, so the badge follows whichever
// reader the funnel configures. `enabled` is off in the editor preview — the
// admin's own tab title should stay put while they work.
export function useTabBadge(messages, persona, enabled = true) {
  const avatar = useRef(null);
  const count = useRef(0);
  const baseTitle = `${persona?.name || 'Chat'} — Chat`;
  const avatarSrc = persona?.avatar;
  ...
  useEffect(() => {
    if (!enabled || !avatarSrc) return;
    const img = new Image();
    img.src = avatarSrc;
    img.onload = () => { avatar.current = img; drawFavicon(img, count.current); };
    // A data-URL avatar is same-origin; a remote one would taint the canvas and
    // make toDataURL throw, so the badge falls back to the plain field.
    img.onerror = () => { avatar.current = null; drawFavicon(null, count.current); };
  }, [enabled, avatarSrc]);

  useEffect(() => {
    if (!enabled) return;
    document.title = count.current > 0 ? `(${count.current}) ${baseTitle}` : baseTitle;
    try {
      drawFavicon(avatar.current, count.current);
    } catch {
      /* tainted canvas — leave the favicon alone rather than take the page down */
    }
  }, [messages, enabled, baseTitle]);
